...
// Shared look for every surface this overlay draws (panels, pills, the key modal).
const palette = (dk: boolean) =>
  dk
...
// ---- first-run key setup ----------------------------------------------------
// GET /__key reports (once per dev-server start) that no API key was found;
// this modal offers to take one and POST it back, where the middleware writes
// it into the repo's .dev.vars (local-only, gitignored — never sent anywhere
// else). Freely dismissable: manual setup (env var / editing the file) always
// works, and the prompt won't return until the dev server restarts.
